...
class CoinsManager {
    constructor(quantity) {
        this.coins = [];
        this.score = 0;
        this.scoreLabel = new TextLabel(20, 30, "24px Arial", "white");
        
        for (let i = 0; i < quantity; i++) {
            const x = Math.random() * (canvasWidth - 50);
            const y = Math.random() * (canvasHeight - 50);

            const coin = new Coin(new Vec(x, y),20, 20, null, 8);
    ...
    update(deltaTime, player) {
        // Update animations for all coins
...
class Game {
    ...
    initObjects() {
        ...
        this.coins = new CoinsManager(10); 
        ...
        this.score = new TextLabel(60, 50, "20px Arial", "black");

        this.points = 0;
    }

    draw(ctx) {
        this.score.draw(ctx, `COINS: ${this.points}`);
    ...
    overlay.id = 'gameOverOverlay';
        overlay.innerHTML = '<h1>You Win!</h1><button onclick="location.reload()">Restart</button>';
